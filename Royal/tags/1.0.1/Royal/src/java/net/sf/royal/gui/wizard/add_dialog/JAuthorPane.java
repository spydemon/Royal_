 ...
 */
public class JAuthorPane extends JPanel
{
	 ...
	 */
	public JAuthorPane(JDialog parent)
	{
	...
	private void addAuthorLine()
	{
		...
		JAuthorPane.this.revalidate();
		JAuthorPane.this.repaint();
	}
	...
	public void setAuthorLines(Set<Work> works)
	{
		...
		for(j = 0, i = 0; i < worksTab.length; i++)
		{
			...
			if(JAuthorPane.this.firstLine)
			{
				JAuthorPane.this.firstLine = false;
			}
	...
	public class AuthorActionListener implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent ae)
		{
			...
			{
				JAuthorPane.this.firstLine = false;
				JAuthorPane.this.gbc.gridy++;
				JAuthorPane.this.gbc.gridx=0;
				parent.setSize(parent.getSize().width,
							   parent.getSize().height
							   + JAuthorPane.this.jepEntryList.get(0).getHeight());
				JAuthorPane.this.addAuthorLine();
			}
			else
			{
				parent.setSize(parent.getSize().width,
						   parent.getSize().height
						   - JAuthorPane.this.jepEntryList.get(0).getHeight());
				JAuthorPane.this.removeAuthorLine(jb);
			}
		}
	}
	...
	public class AuthorDetailsListener implements ActionListener
	{
		...
		{
			Author au = PersistencyManager.findAuthorByID(JAuthorPane.this.jepEntryList.get(_r).getID());
			if(au == null)
			{
				au = new Author();
				au.setNickName(JAuthorPane.this.jepEntryList.get(_r).getText());
			}
			...
			if(aad.getID() != null)
			{
				JAuthorPane.this.jepEntryList.get(_r).refresh();
				JAuthorPane.this.jepEntryList.get(_r).setID(aad.getID());
			}
			JAuthorPane.this.parent.toFront();
		}
	}
}
