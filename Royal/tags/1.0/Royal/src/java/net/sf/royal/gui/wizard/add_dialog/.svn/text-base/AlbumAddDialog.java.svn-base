...
// Methods
	...
	private void init()
	{
		...
		this.jbGoogleImage.setEnabled(false);
	...
	private void initListener()
	{
		...
		this.jbImageBrowse.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				{
					File toCopy = ic.getSelectedFile().getAbsoluteFile();
					File dest = new File(PropertyManager.getInstance().getPathProperty("path_cover") + PropertyManager.sep
							  + HibernateUtil.getCurrentDatabase() + PropertyManager.sep + ic.getSelectedFile().getName());

					if(dest.exists())
					{
						dest = new File(PropertyManager.getInstance().getPathProperty("path_cover") + PropertyManager.sep +
								   HibernateUtil.getCurrentDatabase() + PropertyManager.sep + "1-" + ic.getSelectedFile().getName());
					}
					try
					{
